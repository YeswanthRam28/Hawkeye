...
# ------------------------
# ------------------------
# Adapter: Convert MockState to Frontend Overlay Format
# ------------------------
def get_overlays_from_state(w: int, h: int):
    state = get_state()
    
    # Convert Objects (bbox [x1,y1,x2,y2] -> x,y,w,h)
    frontend_boxes = []
    for obj in state.vision_data.get("objects", []):
        bbox = obj["bbox"]
        # Assuming bbox is absolute coords [x1, y1, x2, y2]
        # But mock generator uses small ints 0-200. We need to scale to w,h?
        # The mock generator used random ints. Let's assume they are pixel coords or scale them.
        # Actually mock generator used: [0-100, 0-100, 100-200, 100-200]
        # This is arbitrary. Let's just pass them or scale them to be visible.
        # Since the original used 0-1 float scaling:
        # We'll map the mock coordinates (approx 0-200) to the frame size.
        
        # Simplification: The mock state returns dummy numbers. 
        # For the visual, let's map them to % of the screen to ensure they are visible.
        ...
        # Scale for visibility on 1280x720 if the mock coords are small
        # The mock coords are in range 0-200.
        scale_x = w / 200.0
        scale_y = h / 200.0
    ...
    # Convert Poses (keypoints already x,y)
    frontend_skeletons = []
    for pose in state.vision_data.get("poses", []):
        # mock keypoints are 100-500.
        # we can just use them directly as they fit in 1280x720
        frontend_skeletons.append({
            "id": f"p_{pose['id']}",
            "keypoints": pose["keypoints"]
        })
        
    # Heatmap (mock is 32x32 base64, frontend expects 2D array of floats)
    # The frontend code: "const hm = overlays.heatmap || []"
    # The previous mock returned a 4x8 array of floats.
    # The new mock returns a base64 image path for "raw_heatmap".
    # For now, let's just generate a dummy 4x8 grid for the frontend visual
    # so we don't break the UI, or parse the base64 (too complex for now).
    heatmap = [[ 0.5 for _ in range(8) ] for __ in range(4)]
    
    return {"boxes": frontend_boxes, "skeletons": frontend_skeletons, "heatmap": heatmap}
